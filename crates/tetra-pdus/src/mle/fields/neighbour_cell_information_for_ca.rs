...
impl NeighbourCellInformationForCa {
    pub fn from_bitbuf(buf: &mut BitBuffer) -> Result<Self, PduParseErr> {
        ...
        let main_carrier_number_extension =
            typed::parse_type2_generic(true, buf, 10, "main_carrier_number_extension")?.map(|v| v as u16);
        let mcc = typed::parse_type2_generic(true, buf, 10, "mcc")?.map(|v| v as u16);
        let mnc = typed::parse_type2_generic(true, buf, 14, "mnc")?.map(|v| v as u16);
        let location_area = typed::parse_type2_generic(true, buf, 14, "location_area")?.map(|v| v as u16);
        let maximum_ms_transmit_power =
            typed::parse_type2_generic(true, buf, 3, "maximum_ms_transmit_power")?.map(|v| v as u8);
        let minimum_rx_access_level =
            typed::parse_type2_generic(true, buf, 4, "minimum_rx_access_level")?.map(|v| v as u8);
        let subscriber_class = typed::parse_type2_generic(true, buf, 16, "subscriber_class")?.map(|v| v as u16);
        let bs_service_details = typed::parse_type2_struct(true, buf, BsServiceDetails::from_bitbuf)?;
        let timeshare_cell_information_or_security_parameters =
            typed::parse_type2_generic(true, buf, 5, "timeshare_cell_information_or_security_parameters")?.map(|v| v as u8);
        let tdma_frame_offset = typed::parse_type2_generic(true, buf, 6, "tdma_frame_offset")?.map(|v| v as u8);
    ...
    pub fn to_bitbuf(&self, buf: &mut BitBuffer) -> Result<(), PduParseErr> {
        ...
        typed::write_type2_generic(true, buf, self.mcc.map(|v| v as u64), 10);
        typed::write_type2_generic(true, buf, self.mnc.map(|v| v as u64), 14);
        typed::write_type2_generic(true, buf, self.location_area.map(|v| v as u64), 14);
        typed::write_type2_generic(
            true,
            buf,
            self.maximum_ms_transmit_power.map(|v| v as u64),
        ...
        typed::write_type2_generic(true, buf, self.subscriber_class.map(|v| v as u64), 16);
        typed::write_type2_struct(true, buf, &self.bs_service_details, |val, inner| {
            val.to_bitbuf(inner);
            Ok(())
        })?;
        typed::write_type2_generic(
        ...
        typed::write_type2_generic(true, buf, self.tdma_frame_offset.map(|v| v as u64), 6);

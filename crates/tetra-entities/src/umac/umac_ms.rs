...
impl UmacMs {
    ...
    /// MM layer retransmits the whole demand). The granted slot is the next
    /// uplink opportunity, i.e. the fixed DL+2 duplex spacing (cl. 9.5) from the
    /// MAC-RESOURCE carrying the grant.
    fn emit_mac_end_hu(&mut self, queue: &mut MessageQueue, grant: Option<&BasicSlotgrant>) {
        ...
        // The granted slot is the next uplink opportunity: the fixed DL+2 duplex
        // spacing (cl. 9.5), the same offset the random access uses.
        ...
        // (cl. 23.5.2) to accept it. The current MS PHY reschedules every uplink
        // to a later reachable occurrence of the opportunity to overcome RX-
        // pipeline latency (see PhyMs::schedule_uplink_time), which changes the
        // frame number and so will not satisfy that exact-slot check until MS
        // timing advance (cl. 9.5) is implemented. The PDU construction and
        // grant handling here are spec-correct and unit-tested regardless; on-
        // air reassembly of the fragmented transfer awaits the timing-advance
        // work. Emitting it via the normal uplink path keeps the whole chain
        // exercised and observable on hardware.
        let blk = TmvUnitdataReq {
            mac_block: block,
